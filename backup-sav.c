...
int main(int argc, char **argv) {
  ...
  printf("LSDj Save Backup by DEFENSE MECHANISM\n");
  printf("based on GBxCart RW v1.24 by insideGadgets\n");
  printf("################################\n");
  ...
  while (inLoop == true) {
      ...
      if (cartridgeMode == GB_MODE) {
        ...
        if (ramEndAddress > 0) {
          ...
          if (confirmWrite == 'y') {
            ...
            // Check if Gameboy Camera cart with v1.0/1.1 PCB with R1 firmware,
            // read data slower
            if (cartridgeType == 252 && gbxcartFirmwareVersion == 1) {
              ...
              for (uint8_t bank = 0; bank < ramBanks; bank++) {
                ...
                set_number(ramAddress,
                           SET_START_ADDRESS); // Set start address again

                RS232_cputs(cport_nr, "M0"); // Disable CS/RD/WR/CS2-RST from
                                             // going high after each command
                ...
                while (ramAddress < ramEndAddress) {
                  for (uint8_t x = 0; x < 64; x++) {
                    ...
                    RS232_cputs(cport_nr,
                                "LD0x60"); // cs_mreqPin_low + rdPin_low
                    RS232_SendByte(cport_nr, 0);
                    RS232_drain(cport_nr);
                    ...
                    RS232_cputs(cport_nr,
                                "HD0x60"); // cs_mreqPin_high + rdPin_high
                    RS232_SendByte(cport_nr, 0);
                    RS232_drain(cport_nr);
              ...
              for (uint8_t bank = 0; bank < ramBanks; bank++) {
                ...
                while (ramAddress < ramEndAddress) {
                  ...
                  } else { // Didn't receive 64 bytes, usually this only happens
                           // for Apple MACs
                    fflush(ramFile);
                    com_read_stop();
                    delay_ms(500);
    ...
    // Erase save
      ...
      printf("*** This will erase the save game from your Gameboy/Gameboy "
             "Advance Cart ***");
      printf("\nPress y to continue or any other key to abort.\n");
      ...
      if (confirmWrite == 'y') {
        ...
        if (cartridgeMode == GB_MODE) {
          ...
          if (ramEndAddress > 0 || eepromEndAddress > 0) {
            ...
            // Before erasing, make a .info file with the memory details as we
            // won't be able to automatically detect it anymore Check if file
            // already exists
            write_cart_ram_info();
                ...
                if (hasFlashSave == FLASH_FOUND_ATMEL) {
                  ...
                  while (currAddr < endAddr) {
                    if (currAddr % 4096 == 0) {
                      ...
                      // Wait for first byte to be 0xFF, that's when we know the
    ...
    // Custom commands
      ...
      while (1) {
        ...
        for (uint8_t x = 0; x < strlen(readInput); x++) {
          if (readInput[x] >= 97 && readInput[x] <= 122 &&
              readInput[x] != 120) {
            readInput[x] -= 32;
          }
        }
        ...
        if (readInput[0] == 'I' || readInput[0] == 'O' || readInput[0] == 'L' ||
            readInput[0] == 'H') {
          RS232_SendByte(cport_nr, 0);
          RS232_drain(cport_nr);
        }
    ...
    // Other options
      ...
      if (otherOptionSelected == '1') {
        ...
        while (1) {
          ...
          if (romBase <= 0x0F) {
            printf("\nSelecting ROM Base 0x0%X at Address: 0x0%X, Size: "
                   "%iKByte, ROM Mask 0x%X\n",
                   romBase, readAddress, (1 << romSize) * 32, romMask);
          } else {
            printf("\nSelecting ROM Base 0x%X at Address: 0x%X, Size: %iKByte, "
                   "ROM Mask 0x%X\n",
                   romBase, readAddress, (1 << romSize) * 32, romMask);
          }
          ...
          RS232_cputs(cport_nr, "M0"); // Disable CS/RD/WR/CS2-RST from going
                                       // high after each command
          RS232_drain(cport_nr);
          delay_ms(5);
          ...
          if (cartridgeMode == GB_MODE) {
            ...
            for (uint8_t bank = 1; bank < romBanks; bank++) {
              if (cartridgeType >= 5) { // MBC2 and above
                ...
                set_bank(0x4000,
                         bank >> 5); // Set bits 5 & 6 (01100000) of ROM bank
                set_bank(0x2000,
        ...
        printf("\n--- GBA Flash cart ROM mapper ---\n"
               "Used for mapping ROMs to 0x00 on GBA flash carts like \"24 in "
               "1\" ones.\n"
               "There are 2 address data bytes to set and ROM size. Most of "
               "the time data bytes are in multiples of 8 (and can be 0 too).\n"
               "E.g, Address 2 set to 0x30 and Address 3 set to 0x28 gives "
               "\"Ice Age\" game.\n"
               "Type x to exit");
        ...
        while (1) {
          ...
          // ROM size
          printf("\nEnter the ROM size in Mbytes to dump (or any key to "
                 "cancel)\n");
          printf(">");
      ...
      // GB "22 in 1" Bank Reader
        ...
        fclose(romFile);
        printf("\nFinished. You must now power cycle GBxCart RW to dump the "
               "other banks.\n");
      }
    }
